...
/*
    1. 사전 지식 : API 와 REST API
        ...
        식당으로 알아보는 API
            내가 손님인데, 식당 들어가면 -> 점원에게 요리를 주문함(주방으로 가서 주문하는게 아님)
            그리고 점원은 주방으로가서 요리 만들어달라고 요청
                요 청         요 청
            손님 <----> 점원 <----> 주방
                응 답         응 답

            의 형태를 띄고 있는데, 여기서 손님은 클라이언트(client), 주방에서 일하는 요리사를 서버(server)라고 생각하시면 됨.
            그리고 중간에 있는 점원을 API 라고 보면 됩니다. -> '매개체' 라고 한점 기억

            우리는 웹 사이트의 주소를 입력해서 '구글 메인 화면을 보여줘' 라고 요청하면, API는 이 요청을 받아서 서버에게 전달.
            그러면 서버는 API가 준 요청을 처리해 결과물을 구성해서 이것을 다시 API로 전달하고, API는 최종 결과물은 브라우저에
            보내주면 화면을 볼 수 있게됨.
            ...
            웹의 장점을 최대한 활용하는 형태라고 알려진 REST API
                Representational State Transfer를 줄일 표현으로, 자원을 이름으로 구분해 자원의 상태를 주고 받는 API 방식
                ...
                REST API를 사용하는 방법
                    규칙 1. URL에는 동사를 쓰지 말고, 자원을 표시한다.
                        *자원 : 가져오는 데이터를 의미. 예를들어 학생 중에 id가 1인 학생의 정보를 가져오는 URL은
                        1)예시 : /students/1
                        2)예시 : /get-student?student_id=1
                    과 같은 방식으로 설계할 수 있는데,

                    이중 더 REST API에 맞는 형식은 1)예시에 해당함. 2)예시의 경우 자원이 아닌 다른 표현을 섞어 사용했기 때문(get).
                    ...
                    규칙 2. 동사는 HTTP 메서드로
                        HTTP 메서드 : 서버에 요청을 하는 방법을 나누는 것. -> POST, GET, PUT, DELETE
                            만들고, 일고, 업데이트하고, 삭제한다 Create / Read / Update / Delete라고 해서 CRUD라고 합니다.
                        예를 들어 블로그에 글을 쓰는 설계를 한다고 가정했을 때
                                ...
                                2) 블로그 글을 '수정'하는 API         : PUT/articles/1
                                2) 블로그 글을 '삭제'하는 API         : DELETE/articles/1

                            * GET / POST 등은 URL에 입력하는 값이 아니라 내부적으로 처리하는 방식을 미리 정하는 것으로
                            실제로 HTTP 메서드는 내부에서 서로 다른 함수로 처리하는데, 대놓고 적는 일은 잘 없습니다.

                            이외에도 '/' 는 계층관계를 나타내는 데 사용하거나, 밑줄 대신 하이픈을 사용하거나, 자원의 종류가
                            컬렉션인지 도큐먼트인지에 따라 단수, 복수를 나누거나 하는 등의 규칙이 있지만 마찬가지로 추후 설명 에정.

                2. 블로그 개발을 위한 엔티티 구성하기
                    이제 프로젝트 시작할 예정인데,

                    엔티티 구성하겠습니다.
                    만들 엔티티와 매핑되는 테이블 구조는 다음과 같이 작성할 예정입니다.
                    +-------------------------------------------------------------+
