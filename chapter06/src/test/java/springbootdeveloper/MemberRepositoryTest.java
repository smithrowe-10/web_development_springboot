...
class MemberRepositoryTest {
    ...
    void getAllMembers(){
        ...
        // memberRepository 는 상속받은 메서드명을 그대로 가져가니까
    ...
    /*
        ...
        member 라는 테이블의 모든 컬럼을 조회하는데, id가 2인 객체의 값들을 보여달라.
    ...
    /*
        이제는 id가 아니라 name 로 찾기로 해보겠습니다.
        id = 기본키
        name 의 경우에는 컬럼이 있을 수도 없을 수도 있기 때문에 JPA 에서 기본으로 name 을 찾아주는
        메서드가 없습니다.

        필수 컬럼이 아닌것을 조건으로 검색을 하기 위해서는 MemberRepository.java 에서
        메서드를 추가할 필요가 있습니다.
    ...
    /*
        이상과 같이 MemberRepository.java 에 method 를 정의하는 것을 '쿼리 메서드'
    ...
    @Test       // @Spl 애너테이션을 달지 않습니다 이 메서드에서 1, 'A'에 해당하는 객체를 저장할거라서
    void  saveMember(){
        //given
        Member member = new Member(1L, "A");

        // when
        memberRepository.save(member);      //save 가 자동완성됐다는 점에도 주목하셔야 합니다.
    ...
    /*
        ...
        만약 여러 엔티티를 한꺼번에 저장하고 싶다면 saveALL 메서드를 사용가능
     */

    @Test
    void saveMembers(){
        //given
        List<Member> members = List.of(new Member(2L, "서민재"),
                new Member(3L, "아르테타"));
        //when
    ...
    void deleteMemberById(){
        //when
        memberRepository.deleteById(2L);        // 여기서 이미 삭제가 끝남
        //then                                -> 2L에 해당하는 객체를 찾았을 때 empty 인지 확인
    ...
    /*
        그런데 지금 이상의 메서드에는 @Transactional 애너테이션이 존재하지 않음.
            -> @DataJpaTest 애너테이션에 @Transactional 이 있기 때문임 T^T
     */
