...
import me.seominjae.springbootdeveloper.repository.BlogRepository;
...
class BlogApiControllerTest {
    ...
    /*
        ...
        : 이 클래스로 만든 자바 객체를 JSON 데이터로 변환하는 것 -> 직렬화
            ...
            HTTP 에서는 JSON 데이터를 사용하고 , 자바에서는 객체를 사용하는데, 서로 형식이 다르기 때문에
            형식에 맞게 변환하는 과정이 필요. 이 작업을 직렬화, 역직렬화 라고함

                1) 직렬화 Java시스템 내부에서 사용되는 객체를 외부에서 사용하도록
                    변환하는 과정을 뜻하는데,
                    예를들어 title은 "제목", content 는 "내용"이라는 값이 들어가있는
                    개체가 있다고 가정했을 때,
                    자바 상에서는
    ...
    public void addArticle() throws Exception {
        ...
        // then
        result.andExpect(status().isCreated());             //201 Created 인지 확인

        List<Article> articles = blogRepository.findAll();  //repository 에서 전체 글(객체)을 가져와 list에 담음

        assertThat(articles.size()).isEqualTo(1);   //list내의 element개수 확인해서 그게 1이면 true

        //list인 articles를 확인해서 가장 첫번째 객체를 꺼내서 확인했더니 String "title"인지 확인
        assertThat(articles.get(0).getTitle()).isEqualTo("title");
        assertThat(articles.get(0).getContent()).isEqualTo("content");

        /*
            List 에서 <> 제네릭 Article 클래스의 객체를 요소로 담게됩니다.
            List 주요 메서드
            ...
            그 이후에 MockMvc를 사용해 HTTP 메서드, URL, 요청본문, 요청타입 등을 설정 뒤 테스트 요청 보냈습니다.
    ...
    public void findArticle() throws Exception{
        ...
        resultActions.andExpect(status().isOk())
                .andExpect(jsonPath("$[0].제목").value(title))
                .andExpect(jsonPath("$[0].내용").value(content));
    }
    /*
        삭제 구현하겠습니다.
    ...
    public void deleteArticle() throws Exception{
        ...
        //가지고 온 List 에 아무런 값이 없는지 확인 -> 왜 아무것도 없냐?
        //Test 메서드는 메서드 단위로 실횅
        //given 단계에서 생성한 객체 하나만 있고 그것을 삭제 했기 때문에
        //articles 내에는 아무런 값도 없어야함
