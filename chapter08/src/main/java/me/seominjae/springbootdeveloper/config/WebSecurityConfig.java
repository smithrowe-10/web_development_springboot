package me.seominjae.springbootdeveloper.config;

import lombok.RequiredArgsConstructor;
import me.seominjae.springbootdeveloper.service.UserDetailService;
...
/*
    1. 스프링 시큐리티의 모든 기능을 사용하지 않게 설정하는 코드
        즉, 인증 인가 서비스를 모든 곳에 적용하진 않음.
        일반적으로 정적 리소스(이미지, html 파일) 에 설정.
        정적 리소스만 스프링 시큐리티 사용을 비활서오하 하는 static 하위 경로에 있는 리소스와
    ...
    3. 특정 경로에 대한 엑세스 설정
        ...
        authenticated() : 별도의 인가는 필요하지 않지만 인증이 성공된 상태여야 잡근 가능
    ...
    5. 로그아웃 설정.
        ...
        invalidHttpSesstion() : 로그아웃 이후에 세션을 전체 삭제할지 여부를 설정

    6. csrf 설정을 비활성화 - CSRF 공격을 방지하기 위해서 사용하는건데, 실습 좀 편하게 하려고 비활성화 해놓음

    7. 인증 관리자 관련 설정. 사용자 정보를 가져올 서비스를 재정의하거나, 인증 방법, 예을 들어
