...
public class BlogService {
    ...
    public Article update(long id, UpdateArticleRequest request) {
        ...
        // domain -> Article 에서 정의한 update를 사용
    ...
    /*
        트랜잭션 - 데이터베이스에서 데이터를 바꾸기 위한 작업 단위를 의미함.
            ...
            예를 들어 계좌 이체를 할 때 이런 과정을 거친다고 가정합니다.
                1) A 계좌에서 출금
                2) B 계좌에 입금
            그런데 1)은 성공했는데, 도중에 2)가 실패하면 고객 입장에서 출금은 됐는데 입금이 안된 상황이 발생
            이런 상황을 방지하기 위해 출금과 입금을 하나의 작업 단위로 묶어서, 트랙잭션이라는 단위로 묶어서
            두 작업을 한 단위로 묶어서 실행하면 됩니다.
            중간에 실패한다면 트랜잭션의 처음 상태로 모두 되돌리면 됩니다.(ROLLBACK)
