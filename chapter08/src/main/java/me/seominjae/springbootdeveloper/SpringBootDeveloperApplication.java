    ...
    학습 목표 : 로그인 / 회원 가입 기능을 구현
        ...
        1. 스프링 시큐리티(Spring Security)
            ...
            3) 필터 기반으로 동작하는 스프링 시큐리티
                스프링 시큐리티의 풀 로그인 과정을 설정하는 건 그렇게 어렵지 않을건데,
                내부적으로 아까 본 그림처럼 꽤 복잡한 과정을 거침

                모두 외워야 하는건 아니지만 로그인 과정이 어떤흐름으로 이어지는지 이해할 필요가 있음.
                일단은 회원 가입과 로그인 관련 부분을 코딩으로 풀어나갈 예정

        2. 회원 도메인 만들기
            스프링 시큐리티를 활용한 인증, 인가 기능 구현 예정.
            회원 정보를 저장할 테이블을 만들고,
            테이블과 연결할 도메인을 만들고,
            이 테이블과 연결할회원 엔티티를 만들 겁니다.
