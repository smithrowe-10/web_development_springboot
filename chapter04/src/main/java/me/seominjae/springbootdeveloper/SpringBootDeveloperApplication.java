...
/*
    1. 테스트 코드 개념 익히기
        ...
        장점 :
            1) 유지 보수에 편리
            2) 코드 수정 시 기존 기능이 제대로 작동되지 않을까봐 걱정할 필요 없음
        ...
        테스트 코드란?
            ...
            ex) 새로운 메뉴를 저장하는 코드를 테스트한다고 가정했을 때의
                ...
                public void savedMenuTest() {
                    ...
                    assertThat(savedMenu.getJPrice()).isEqualTo(price);
                }
                ...
                JUnit 이란?
                    ...
                    특징 :
                        ...
                        2) @Test 애너테이션으로 메서드를 호출할 때 마다 새 인스턴스를 생성,
                            독립테스트가 가능
                        3) 에상 결과를 검증하는 assertion 메서드 제공
                        4) 사용법이 단순, 테스트 코드 작성시간이 적음(익숙해지면)
                        5) 자동실행, 자체 결과를 확인하고 실패지점 보여줌

        test/java 폴더에 JUnitTest.java 파일을 생성

        JUnit 클래스에 예시

        AssertJ로 검증된 가독성 높이기

            AssertJ는 JUnit과 함께 사용해 검증문의 가독성을 높여주는 라이브러리
            에를 들어서 아까 작성한(JUnitTest.java에서)(
            Assertions.assertEquals(sum, a+b); 를 보시면 기대값과 비교값이 잘 구분되지 않습니다.

            AssertJ를 사용한 코드 라인
            assertThat(a+b).isEqualTo(sum);
            이 경우 a와 b를 더한 값이 sum 과 같아야 한다는 의미로 읽힙니다 - 영어의 경우
            ...
            4) doesNotContain(A) : A를 포함하지않는지 검증
            5) startsWith(A) : 점두사가 A인지 검증
            ...
            테스트 코드 작성 연습 문제 풀어보기 - test/java 폴더에 JUnitQuiz 파일
