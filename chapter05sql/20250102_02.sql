...
-- 
-- 예를 들어 실습 데이터에서 회원 수를 구하려면 어떻게해야할까, 전체 회원 수가 아니라,
-- 국가별 회원 수를 구하는 방법, 제품별 매출을 구하는 바업ㅂ 등을 실제로 알아볼 수 있음.
...
SELECT COUNT(country)	-- country 컬럼에서 null이 아닌 값의 계수, 즉 중복 계산
	from users
	;
...
-- country의 데이터값의 종류를 계산하고 싶다면 어떻게해야할까?
...
SELECT ROUND(AVG(PRICE),2) as "avgPrice"	-- ROUND(집계함수결과, 소숫점자리)
	from PRODUCTS 
	;
...
ROUND(컬럼명, 소수점자리수) : 지정 소수점 자리까지 반올림하여 반환
SUBSTR(컬럼명, 시작위치, 가져올문자개수) ; 문자열을 지정한 시작 위치부터 지정한 문자 개수만큼
...
이상을 이유로 집게 함수는 SELECT 에서만 사용 가능
...
-- 예를 들어, 전체회원수가 아닌 국가별 회원수를 나누어 계산하기도 하며,
-- 월별로 가입한 회원수를 집계하기도 하기 때문.
-- 즉, 특정 조건으로 나눈 그룹별 계산을 수행할 때는 먼저 그룹화를 선행해야 합니다.
-- 그후 집계 함수는 특정 컬럼을 기준으로 데이터를 그룹화 한 후에, 그룹별로 집계 함수를 적용해야 합니다.
...
-- SQL문을 작성해야 합니다.
-- 국가수는 적지도 않고, 실제 환경에서는 훨씬 더 많은 데이터를 다루는데, 조건을 일일이
-- 즉, where 절에서 country =의 값을 매번 바꾸는 방식은 사용하지 않습니다.
