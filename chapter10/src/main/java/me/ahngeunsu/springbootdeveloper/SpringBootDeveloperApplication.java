    ...
    사전 지식 : 0Auth
        ...
        관련 용어 정리
            ...
            리소스 서버(Resource Server) : 리소스 오너의 정보를 가지며, 리소스 오너의 정보를
                보호하는 주체를 의미하고, 네이버, 구글, 카카오, 페이스북,이 리소스 서버에 해당
            인증 서버(Authorization Server) : 클라이언트에 리소스 오너의 정보레 접근할 수 있는
                토큰을 발급하는 역할을 하는 애플리케이션을 의미
            클라이언트 애플리케이션(Client Application) : 인증 서버에게 인증을 받고 리소스 오너의
                리소스를 사용하는 주체를 의미.

    OAuth 를 사용하면 인증 서버에서 발급받은 토큰을 사용해서 리소스서버에 리소스오너의 정보를 요청하고
        응답 받아 사용가능
        그렇다면 리로스오너의 정보를 어떻게 취득할 수 있는지에 대한 방법이 필요.
        ...
        리소스 오너 정보를 취득하는 네 가지 방법
            ...
            2) 암시적 승인(Implicit Grant Type) :
                서버가 없는 자바스크립트 웹 애플리케이션에서 클라이언트에서 주로 사용.
            ...
            3) 리소스 소유자 암호 자격증명 승인 타입(Resource Owner Password Credentials) :
                클라이언트의 패스워드를 이요해서 액세스 토큰에 대한 사용자의 자격 증명을 교환하는 방식
            ...
            권한 부여 코드 승인 타입이란?
                ...
                애플리케이션 ----------------------------→ 인증 서버
                    ...
                    |     \                                 |
                    |      \                                |
   1.  권한요청      |       5. 액세스 토큰으로 데이터에 접근     | 인증 서버에
                    |                              \        | 인증 작업 위임
                    |                               \       |
                    |                                \      |
                    |                                 \     |
    ...
    쿠키 관리 클래스 구현.
        ...
        springbootdeveloper 패키지 내에 util 패키지를 생성 -> CookieUtil.java
 */