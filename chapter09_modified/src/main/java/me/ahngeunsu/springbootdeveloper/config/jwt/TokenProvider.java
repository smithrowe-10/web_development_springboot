...
public class TokenProvider {
    ...
    private String makeToken(Date expiry, User user) {
        ...
        return Jwts.builder()
                .setHeaderParam(Header.TYPE,Header.JWT_TYPE)    // 헤더 typ : JWT 고정
                // 내용 iss : mayveags@gmail.com(JwtPropertie에서 설정함)
                .setIssuer(jwtProperties.getIssuer())
                .setIssuedAt(now)               // 내용 iat : 현재 시간
                .setExpiration(expiry)          // 내용 exp : expiry 멤버 변수값
                .setSubject(user.getEmail())    // 내용 sub : 유저 이메일
                .claim("id", user.getId())   // 플레임 id : 유저 ID
...
/*
    1. makeToken() :
        토큰을 생성하는 메서드 argument 로 만료시간, 유저정보를 받았습니다.
        set 계열 메서드들을 통해 여러 값을 지정햇는데
    ...
    3. getAuthentication() :
        토큰을 받아 인증 정보를 담은 객체 Authentication 을 반환하는 메서드 프로퍼티즈 파일에 저장한
        비밀값으로 토큰을 복호화한 뒤 클레임을 가져오는 private 메서드인 getClaims()를 호출해서
        클레임 정보를 반환 받아 사용자 이메일이 들어있는 토큰 제목 sub와 토큰 기반으로 인증증 정보 생성
        이때 UsernamePAsswordAuthenticationToken의 첫 argument로 들어가는 User는 프로젝트에서
        만든 클래스가 아닌, 스프링 시큐리티에서 제공하는 객체인 User 클래스를 import해야함.

    4. getUserId() :
        토큰 기반으로 사용자 ID를 가져오는 메서드. 프로퍼티즈 파일에 저장한 비밀값으로 토큰을 복호화한 다음
        클레임이 가져오는 private getClaims()를 호출해서 클레임을 정보를 반환받고 클레임 id 키로 저장된
        값을 가져와 반환.

        다음 단계 -> 코드가 제대로 동장하는지 확인하기 위해 테스트 코드 작성 예정
